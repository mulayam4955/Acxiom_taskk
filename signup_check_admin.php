...
if (isset($_POST['uname']) && isset($_POST['password']) && 
    ...
    if (empty($uname)) {
        ...
        header("Location: admin_signup.php?error=Name is required&$user_data");
        exit();
        ...
        if (mysqli_num_rows($result) >0){
            header("Location: admin_signup.php?error=The User Name is already taken try another&$user_data");
            exit();
        }else{
           $sql2 = "INSERT INTO users(user_name, password, name) VALUES('$uname', '$pass', '$name')";
           $result2 = mysqli_query($conn, $sql2);
           if ($result2) {
                header("Location: admin_signup.php?success=Your account has been created successfully&$user_data");
                exit();
           }else {
                header("Location: admin_signup.php?error=Error Occured&$user_data");
                exit();
           }
        }
